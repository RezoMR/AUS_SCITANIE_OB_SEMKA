...
namespace structures
{
	...
	Structure& PriorityQueueTwoLists<T>::assign(Structure& other)
	{
		...
		PriorityQueueTwoLists<T>& otherTL = dynamic_cast<PriorityQueueTwoLists<T>&>(other);
		shortList_->assign(*otherTL.shortList_);

		for (auto item : *otherTL.longList_) {
	...
	void PriorityQueueTwoLists<T>::push(int priority, const T& data)
	{
		if (shortList_->getCapacity() != shortList_->size()) {
			...
			PriorityQueueItem<T>* removed = shortList_->pushAndRemove(priority, data);
			longList_->add(removed);

		}
		else {
			PriorityQueueItem<T>* removed = new PriorityQueueItem<T>(priority, data);
			longList_->add(removed);
	...
	T PriorityQueueTwoLists<T>::pop()
	{
		...
		if (shortList_->isEmpty() && longList_->size() != 0) {
			...
			LinkedList<PriorityQueueItem<T>*>* auxiliary = new LinkedList<PriorityQueueItem<T>*>();
			...
			while (!longList_->isEmpty()) {
				...
				if (tempVar != nullptr) {
					auxiliary->add(tempVar);
				}
			}

			longList_->clear();
			longList_->assign(*auxiliary);
